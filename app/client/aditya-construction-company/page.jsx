...
export default function RealEstatePreview() {
  return (
    <div className={`min-h-screen ${"bg-gray-900 bg-gradient-to-br from-gray-900 to-blue-900"} font-sans overflow-hidden`}>
      {/* Navigation */}
      <nav className="p-6 md:px-12 flex justify-between items-center z-50 relative">
        <div className="flex items-center gap-3">
          ...
          <span className={`text-xl font-bold ${"text-white"}`}>Aditya Construction Company</span>
        </div>
        <button className={`px-6 py-2.5 rounded-lg font-semibold ${"bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg shadow-blue-500/30 hover:scale-105 transition-all"}`}>
          Client Portal Login
        </button>
      </nav>
      ...
      <main className="px-6 md:px-12 py-16 md:py-32 max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
        <div className="space-y-8">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold ${"bg-white/10 backdrop-blur-lg border border-white/20 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]"} ${"text-blue-200"}`}>
          ...
          <h1 className={`text-5xl md:text-7xl leading-tight ${"text-white"}`}>
            ...
            <span className={`${"text-blue-200"}`}>Digitized.</span>
          </h1>
          <p className={`text-lg md:text-xl max-w-lg opacity-80 ${"text-white"}`}>
            Welcome to the new digital home of Aditya Construction Company. We've integrated VR tours, smart CRM, and advanced listing management directly into your operations.
          </p>
          <div className="flex gap-4">
            <button className={`px-8 py-4 rounded-xl font-bold flex items-center gap-2 ${"bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg shadow-blue-500/30 hover:scale-105 transition-all"}`}>
