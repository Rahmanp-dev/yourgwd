...
export default function ClientPage() {
  ...
  const iconMap = {
    Heart: <Heart size={36} color="#2563eb" />,
    Activity: <Activity size={36} color="#2563eb" />,
    Users: <Users size={36} color="#2563eb" />,
    Zap: <Zap size={36} color="#2563eb" />,
    Leaf: <Leaf size={36} color="#2563eb" />,
    FileText: <FileText size={36} color="#2563eb" />
  };
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', color: '#0f172a', background: '#ffffff', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 8%', borderBottom: '1px solid #f1f5f9' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.45rem', fontWeight: 900, color: '#0f172a' }}>
          <div style={{ width: '44px', height: '44px', background: '#f8fafc', borderRadius: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            {iconMap['Activity'] || <Activity size={36} />}
          </div>
          Brightlands Diagnostics
        </div>
        <div>
          <button style={{ background: '#2563eb', color: '#fff', border: 'none', padding: '10px 24px', borderRadius: '8px', fontWeight: 700, cursor: 'pointer' }}>
            Schedule Blood Test
          </button>
        </div>
      </nav>
      ...
      <header style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #ffffff 100%)', padding: '100px 8%', display: 'flex', alignItems: 'center', gap: '40px', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 500px' }}>
          <div style={{ display: 'inline-block', background: '#dbeafe', color: '#1d4ed8', padding: '6px 16px', borderRadius: '100px', fontSize: '0.85rem', fontWeight: 800, marginBottom: '24px' }}>
            Pathology & Imaging • Tolichowki
          </div>
          <h1 style={{ fontSize: '3.6rem', fontWeight: 900, color: '#0f172a', lineHeight: 1.15, marginBottom: '24px', letterSpacing: '-1.5px' }}>
            Reliable Pathology Tests, <span style={{ color: '#2563eb' }}>At Your Convenience</span>
          </h1>
          <p style={{ fontSize: '1.25rem', color: '#4b5563', lineHeight: 1.6, marginBottom: '40px', maxWidth: '600px' }}>
            Serving Nanal Nagar with state-of-the-art pathology, fast biochemistry profiles, and certified clinical diagnostics.
          </p>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleBook} style={{ background: '#ffffff', padding: '30px', borderRadius: '20px', border: '1px solid #e5e7eb', boxShadow: '0 10px 30px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '500px' }}>
              ...
              <button type="submit" style={{ width: '100%', padding: '16px', background: '#2563eb', color: '#fff', borderRadius: '8px', border: 'none', fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                Schedule Blood Test
              </button>
            </form>
          )}
        ...
        <div style={{ flex: '1 1 400px', display: 'flex', justifyContent: 'center' }}>
          <div style={{ width: '380px', height: '380px', borderRadius: '30px', background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
      ...
      <section style={{ padding: '80px 8%', background: '#f9fafb' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '1.1rem', textAlign: 'center', marginBottom: '60px' }}>
          Custom tailored features designed explicitly for the medical operations of Brightlands Diagnostics.
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', maxWidth: '1200px', margin: '0 auto' }}>
          {[{"name":"Book Home Collection","desc":"Schedule a certified phlebotomist to collect blood samples at your home."},{"name":"12-Hour Digital Delivery","desc":"Certified PDF reports sent securely via SMS, WhatsApp, and Email."},{"name":"Wellness Test Packages","desc":"Book bundles for blood sugar, thyroid, and liver panels."}].map((feat, i) => (
      ...
      <footer style={{ background: '#0f172a', color: '#9ca3af', padding: '60px 8%', textAlign: 'center', borderTop: '1px solid #1f2937' }}>
        <h3 style={{ color: '#fff', fontSize: '1.4rem', fontWeight: 800, marginBottom: '8px' }}>Brightlands Diagnostics</h3>
