...
async function run() {
  ...
  const searchQueries = [
    'family dentist Georgetown TX',
    'pediatric dentist Georgetown TX',
    'cosmetic dentist Georgetown TX',
    'oral surgeon Georgetown TX',
    'dental office Georgetown TX',
    'dentistry Georgetown TX',
    'endodontist Georgetown TX',
    'periodontist Georgetown TX',
    'dentist Georgetown Texas'
  ];
  ...
  for (let i = 0; i < placesArray.length; i++) {
    ...
    try {
      ...
      if (isGeorgetown && hasPhone && hasNoWebsite) {
        ...
        console.log(`  -> ADDED [${noWebsiteClinics.length}]: ${name} | Phone: ${details.phone} | Website: ${details.website}`);
      }
    } catch (err) {}
  }
