...
async function run() {
  ...
  const searchQueries = [
    'dentist Georgetown TX',
    'dental clinic Georgetown TX',
    'family dentistry Georgetown TX',
    'orthodontist Georgetown TX',
    'pediatric dentist Georgetown TX',
    'cosmetic dentistry Georgetown TX',
    'endodontics Georgetown TX',
    'periodontics Georgetown TX',
    'oral surgeon Georgetown TX',
    'dental office Georgetown TX'
  ];
  ...
  for (let i = 0; i < placesArray.length; i++) {
    ...
    // Check if we already have 20 clinics
    if (noWebsiteClinics.length >= 20) {
      console.log("Found 20 clinics without website. Stopping scan.");
      break;
    }
  ...
  console.log(`\nFinal list of clinics without websites (${noWebsiteClinics.length}):`);
