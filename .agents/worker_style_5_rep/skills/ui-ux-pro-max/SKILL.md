...
- `disabled-states` - Disabled elements use reduced opacity + cursor change + semantic attribute
- `progressive-disclosure` - Reveal complex options progressively
- `inline-validation` - Validate on blur (not keystroke)
- `input-type-keyboard` - Use semantic input types
...
- `read-only-distinction` - Read-only state should be visually and semantically different
...
- `screen-reader-summary` - Provide a text summary or aria-label describing the chart
